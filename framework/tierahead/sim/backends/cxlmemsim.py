...
WHY THIS MATTERS FOR TierAhead: it is the one CXL-simulation tool in this
...
machine (see tierahead.hw.probe()) is a no-CUDA Apple Silicon Mac -- Darwin
...
HOW TO ACTUALLY RUN THIS (on an x86_64 Linux box with KVM):
  ...
  2. Build/boot the guest per its docs (QEMU CXL device model + guest driver
     shim).
  3. Run tierahead's OWN CPU-only inference entry point (see
     tierahead.collect / a plain `transformers` decode loop for OLMoE, which
  ...
  4. Feed the resulting per-epoch latency/bandwidth measurements back into
     ...
     output onto the SAME ``tierahead.sim.policies.PolicyResult`` shape the
...
from tierahead.sim.policies import PolicyResult
...
def run(*args, **kwargs):
    caps = probe()
    raise RuntimeError(
        "tierahead.sim.backends.cxlmemsim.run() cannot execute on this machine: "
        ...
        "exact steps). Use backend='des' (tierahead.sim.backends.des) for the always-available equivalent, "
