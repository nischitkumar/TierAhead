"""Unified `tierahead` CLI. `tierahead <subcommand> --help` for details on any one.

Subcommands map directly onto Main.md's eight framework components:
  doctor     -- tierahead.hw capability report (what runs here, what needs a GPU/Linux box)
...
def cmd_analyze(args):
    from tierahead.analyze.workload import run_characterization
...
def cmd_sim(args):
    from tierahead.sim.policies import run_concurrency_sweep, run_policy_bakeoff
    from tierahead.traces.io import resolve_trace_dir
    from tierahead.specs import MODEL_SPECS
    data_root = Path(args.data_root) if args.data_root else None
    trace_dir = resolve_trace_dir(MODEL_SPECS[args.model].trace_dir_default, data_root)
...
def cmd_validate(args):
    from tierahead.sim.validation import run_validation_suite
    from tierahead.traces.io import resolve_trace_dir
    from tierahead.specs import MODEL_SPECS
    data_root = Path(args.data_root) if args.data_root else None
    trace_dir = resolve_trace_dir(MODEL_SPECS[args.model].trace_dir_default, data_root)
...
def cmd_kv(args):
    from tierahead.kv.cotenancy import run_kv_expert_sweep, recommend_policy
    data_root = Path(args.data_root) if args.data_root else None
...
def cmd_baseline(args):
    from tierahead.baseline.compare import run_baseline_vs_cxl, run_flagship_report
    data_root = Path(args.data_root) if args.data_root else None
    calib = json.loads(Path(args.calib).read_text()) if args.calib and Path(args.calib).exists() else None
    if args.concurrency_grid:
        grid = [int(x) for x in args.concurrency_grid.split(",")]
...
def cmd_report(args):
    from tierahead.analyze.workload import run_characterization
    from tierahead.report.markdown import df_to_markdown_table, render_report_md
    from tierahead.roofline.model import run_roofline
    ...
    render_report_md(
        title=f"TierAhead report: {args.model}",
...
def main(argv: list[str] | None = None):
    ap = argparse.ArgumentParser(prog="tierahead", description=__doc__,
    ...
    p.add_argument("--concurrency", type=int, default=1,
                    help="N concurrent decode streams sharing the link (fair-share proxy -- see "
                         "tierahead.sim.concurrency for its validated limits)")
    ...
    p.add_argument("--concurrency-grid", default=None,
                    help="comma list, e.g. 1,2,4,8 -- if given, runs the flagship comparison once per "
                         "concurrency level (tierahead.baseline.compare.run_flagship_report) and stacks the "
