...
TierAhead is the production framework for the *Nebula@BITS Goa 2026* (Astera Labs)
...
discipline. Run `tierahead doctor` to see exactly what this means on your
machine:
...
------------------------------------------------------------
trace collection      -> UNAVAILABLE here -- use the pilot committed traces under results/, or run tierahead collect on a CUDA box
...
traces** (`tierahead.collect`) and **real per-layer GPU compute-time
calibration** (`tierahead.calibrate.compute` / `.transfer_measure` /
...
```
                    normal  (HBM-only, no CXL tier at all)
                        │
tierahead.baseline.compare──── the flagship "is this even feasible, and what
                        │     does tiering cost/save" entry point
                        ▼
  results/*/b1/  ──►  [1] tierahead.collect        (CUDA-required)
  traces.jsonl.zst          │
                             ▼
                     [2] tierahead.analyze          Coverage@C / Gini / churn /
                             │                     Recall@m,d / HTB
                             ▼
                     [0] tierahead.roofline         rho = t_transfer / t_compute
                             │
              ┌──────────────┼───────────────────────┐
              ▼              ▼                       ▼
   [3] tierahead.policy.  [4] tierahead.policy.   [4b] tierahead.policy.
       residency            predictor_freq /        precision_gate
       (fixed-C)            predictor_mlp
              │              │                       │
              └──────────────┴───────────┬───────────┘
                                          ▼
                            [5] tierahead.sim  (policy bake-off + N-class DES)
                                          │
                        ┌─────────────────┼─────────────────┐
                        ▼                 ▼                 ▼
             [8] tierahead.kv        [6] tierahead.tco    tierahead.sim.backends
             (KV co-tenancy)       (pooling & TCO)     analytical / des / cxlmemsim
                                                                  │
                                                       [7] tierahead.dashboard
```

| # | Package | What it does | Needs GPU? |
|---|---|---|---|
| 0 | `tierahead.roofline` | ρ = t_transfer/t_compute regime model, provisioning table | No |
| 1 | `tierahead.collect` | Forward-hook router-decision trace collector | **Yes** |
| 2 | `tierahead.analyze` | Coverage@C, Gini/Zipf, churn, Recall@m,d, HTB, verdict | No |
| 3 | `tierahead.policy.residency` | Fixed-residency placement, shared-expert pinning | No |
| 4 | `tierahead.policy.predictor_freq` / `predictor_mlp` | Prefetch engine (frequency table + trainable MLP) | No (CPU/MPS) |
| 4b | `tierahead.policy.precision_gate` | Confidence-gated FP16/NF4/skip fallback | No |
| 5 | `tierahead.sim` | Trace-driven policy bake-off + general N-class fluid DES | No |
| 6 | `tierahead.tco` | Pond-style pooling & stranding, tokens/sec/$ crossover | No |
| 7 | `tierahead.dashboard` | Streamlit + Plotly interactive explorer, 4 tabs | No |
| 8 | `tierahead.kv` | KV-cache/expert link co-tenancy simulation | No |
| — | `tierahead.baseline` | **Flagship**: normal (HBM-only) vs CXL-tiered, side by side | No |
| — | `tierahead.calibrate` | Hardware calibration ladder (compute/transfer/e2e/NF4) | **Yes** (except `policy_check`/`report`) |
...
wherever it's installed. Override with `TIERAHEAD_DATA_ROOT=/path/to/Nebula/Code`
...
tierahead baseline --model olmoe   --residency-pct 25 --bw-gbps 128 --precision fp16 --depth 4
tierahead baseline --model mixtral --residency-pct 75 --bw-gbps 128 --precision nf4  --depth 8
...
dashboard` are shortcuts for the equivalent `tierahead ...` invocations.
...
independently-built simulator. `tierahead sim --model <m> --depth <d>` lets you
...
### `tierahead roofline` — the regime model

```bash
tierahead roofline --model olmoe
tierahead roofline --model mixtral --calib /path/to/calib_compute.json --out sweep.csv
```
...
runs it through `tierahead.roofline.model.validate_calib_sanity` first (see
[PREDICTED.md](PREDICTED.md)'s Methodology section for why that check exists
and what it already caught in this repository).

### `tierahead analyze` — workload characterization

```bash
tierahead analyze --model olmoe --out olmoe_characterization.json
```
...
### `tierahead sim` — the policy bake-off

```bash
tierahead sim --model olmoe --residency-pct 25 --bw-gbps 128 --depth 4 \
...
`tierahead.analyze.workload.recall_at_m`'s docstring for the full derivation and
a hand-worked proof.
...
`tierahead validate --model <m>` runs the seven validation gates this bake-off
...
for any trace on demand (`tierahead.sim.validation.find_collapse_concurrency`).
...
concurrency`. This is what `--concurrency` divides by. `tierahead.sim.concurrency`
...
boundary. The module docstring in `tierahead/sim/concurrency.py` also documents
...
### `tierahead baseline` — the flagship comparison

```bash
tierahead baseline --model mixtral --precision fp16 --hbm-budget-gb 80
...
# One call, every concurrency level at once (tierahead.baseline.compare.run_flagship_report):
tierahead baseline --model olmoe --residency-pct 25 --bw-gbps 128 --precision fp16 --depth 4 \
...
otherwise needs one manual `tierahead baseline` invocation per concurrency
...
accounting via `tierahead.baseline.compare.kv_hbm_pressure`; leaving
...
### `tierahead kv` — KV-cache / expert co-tenancy

```bash
tierahead kv --models olmoe,mixtral --bw-grid 32,64 --policies expert-first,kv-first,weighted:0.5
```

### `tierahead tco` — pooling & TCO

```bash
tierahead tco --n-hosts 128 --group-sizes 1,8,32 --cv 0.4
```

### `tierahead report` — a shareable Markdown summary

```bash
tierahead report --model olmoe --residency-pct 25 --bw-gbps 128 --depth 4 --out olmoe_report.md
```
...
tierahead[dashboard]`. Smoke-tested headlessly via Streamlit's `AppTest`
harness (`from streamlit.testing.v1 import AppTest`) — 0 exceptions across
every tab's default render, no browser needed.

### `tierahead collect` / `tierahead.calibrate.*` — CUDA-required

Every one of these refuses immediately with an actionable message on a
non-CUDA machine (via `tierahead.hw.probe()`), rather than failing deep inside a
`torch.cuda` call:

```bash
# On a CUDA box, from the repo root (not framework/):
python3 -m tierahead.collect.collector --model olmoe --out results/olmoe/b1
python3 -m tierahead.calibrate.compute --model olmoe --out calib_compute.json
python3 -m tierahead.calibrate.compute --model mixtral --quant nf4 --out calib_compute.json
python3 -m tierahead.calibrate.transfer_measure --out calib_transfer.json
python3 -m tierahead.calibrate.e2e_offload --residency-pcts 25,50 --out calib_e2e.json
python3 -m tierahead.calibrate.nf4_fidelity --n-prompts 20 --out calib_nf4_fidelity.json
```
...
through `tierahead.roofline.model.validate_calib_sanity` — this repository's own
...
`tierahead.sim.backends.select_backend("auto"|"analytical"|"des"|"cxlmemsim")`:
...
CXLMemSim" made concrete: `tierahead.baseline.compare.run_baseline_vs_cxl(...,
...
(`tierahead.sim.concurrency`), KV-cache HBM pressure accounting, KV/expert
...
trace file from disk on every single call — harmless for a one-off `tierahead
...
memoized (`tierahead.sim.policies._load_and_split_decode`, keyed by
...
`tierahead/specs/{tiers,models}.py` define in code (the code is the source of
