"""[7] tierMoE interactive dashboard (Main.md sections 4.7 / 7.3's demo
script). Four tabs, matching the E13 dashboard spec exactly:
  ...
  3. Policy explorer   -- live policy bake-off (tiermoe.sim.policies) on the
...
Run with `tiermoe dashboard` (wraps `streamlit run` on this file) or
directly: `streamlit run tiermoe/dashboard/app.py`. Needs the `dashboard`
extra (`pip install tiermoe[dashboard]`). No GPU required -- every number on
...
from tiermoe.roofline.model import bytes_per_layer, get_t_compute_ms, rho as rho_fn, run_roofline
from tiermoe.specs import MODEL_SPECS
from tiermoe.specs.tiers import LINK_BW_GBPS_GRID
from tiermoe.tco.pooling import simulate_fleet_demand, stranding_analysis, tco_crossover
from tiermoe.traces.io import find_data_root, load_decode_df

st.set_page_config(page_title="tierMoE Explorer", layout="wide")
...
def _run_concurrency_sweep_cached(model_tag: str, residency_pct: float, bw_gbps: float, precision: str,
    ...
    from tiermoe.sim.policies import run_concurrency_sweep
    spec = MODEL_SPECS[model_tag]
...
st.title("tierMoE -- Router-Guided Memory Tiering for HBM + CXL MoE Inference")
st.caption(
...
with tab3:
    ...
    if show_sweep:
        ...
        st.caption(
            ...
            "tiermoe.sim.validation.gate_oracle_invariant_to_concurrency). See PREDICTED.md Sec 2.5 "
