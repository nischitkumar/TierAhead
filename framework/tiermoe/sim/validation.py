...
pytest asserts, so `tiermoe doctor` / `tiermoe sim --validate` can run them
...
from tiermoe.sim.policies import _load_and_split_decode, run_concurrency_sweep, simulate_policy, run_policy_bakeoff
from tiermoe.specs import MODEL_SPECS
from tiermoe.specs.tiers import CXL_LATENCY_NS_GRID


def _split(trace_dir: Path, seed: int = 0):
    # Delegates to the memoized loader in tiermoe.sim.policies -- this
