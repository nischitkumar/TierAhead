...
`tiermoe.sim.concurrency`'s module docstring for the full reasoning
...
honest caveats, both favoring under- rather than over-confidence:
  ...
  - `tiermoe.sim.concurrency.max_sustainable_concurrency` computes the point
...
(arbitrary, non-identical traffic) is `tiermoe.sim.engine`, used by
`tiermoe.kv.cotenancy`. See PREDICTED.md's Methodology section.
...
from tiermoe.policy.precision_gate import PrecisionGatePolicy
from tiermoe.policy.predictor_freq import FrequencyTablePredictor
from tiermoe.policy.residency import FixedResidencyPolicy
from tiermoe.roofline.model import get_t_compute_ms
from tiermoe.specs import MODEL_SPECS
from tiermoe.traces.io import expert_counts, explode_topk, load_traces, train_test_split_reqs
...
def simulate_policy(*, decode_train: pd.DataFrame, decode_test: pd.DataFrame, spec, policy: str,
    ...
    if concurrency > 1:
        from tiermoe.sim.concurrency import aggregate_rho, max_sustainable_concurrency
        ...
        # just a proxy (see tiermoe.sim.concurrency's module docstring for
...
def _fit_mlp_predictor(decode_train: pd.DataFrame, n_experts: int, n_layers: int, depth: int, seed: int):
    ...
    richer variant should use tiermoe.policy.lookahead.direct_recall_for_depth
    directly (it has full feature access via features.build_layer_dataset)."""
    try:
        import tiermoe.policy.features as feat
        import tiermoe.policy.predictor_mlp as mlpmod
    except ImportError:
        return None
...
def _load_and_split_decode_cached(trace_dir_str: str, seed: int):
    ...
    `run_concurrency_sweep` and every gate in `tiermoe.sim.validation`. A
...
def run_policy_bakeoff(model_tag: str, trace_dir, policies: list[str] | None = None, residency_pct: float = 25.0,
    ...
    oracle-gap-closed. This is what `tiermoe sim` and the baseline-vs-CXL
