...
-- no GPU, no torch import at module scope, so importing tiermoe.traces never
drags in torch (matters for tiermoe.analyze / tiermoe.roofline / tiermoe.sim,
...
def find_data_root(start: Path | None = None) -> Path:
    ...
    to the TIERMOE_DATA_ROOT env var, then to the current working directory,
    ...
    if "TIERMOE_DATA_ROOT" in os.environ:
        return Path(os.environ["TIERMOE_DATA_ROOT"]).resolve()
...
def load_traces(path: Path) -> pd.DataFrame:
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            f"(see tiermoe.hw.probe().can_collect_traces and README.md's "
