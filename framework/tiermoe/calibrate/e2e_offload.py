...
tiermoe.policy's real (non-oracle) predictors are validated against.
"""
...
def compute_hot_sets(n_layers: int, n_experts: int, residency_pct: float, trace_dir: Path) -> dict:
    from tiermoe.traces.io import load_decode_df
...
def main(argv=None):
    ...
    if not probe().can_run_real_gpu_calibration:
        print("tiermoe calibrate e2e_offload: refusing -- no CUDA on this machine.", file=sys.stderr)
        sys.exit(1)

    import torch
    from tiermoe.collect.prompts import build_workload
