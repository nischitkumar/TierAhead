...
from tiermoe.eunion import e_union_curve
from tiermoe.specs import MODEL_SPECS, LINK_BW_GBPS_GRID
from tiermoe.specs.models import RESIDENCY_PCT_GRID, BATCH_GRID_E1, PRECISION_GRID
from tiermoe.traces.io import find_data_root, load_decode_df, resolve_trace_dir
...
def validate_calib_sanity(calib: dict) -> dict[str, list[str]]:
    ...
    plausible, so callers (tiermoe doctor, the CLI, tests) can decide how
        ...
        if len(means) >= 2 and min(means) > 0:
            ...
            if spread < 0.10:
                w.append(
                    ...
                    f"tiermoe.calibrate.compute on real CUDA hardware."
                )
...
def run_roofline(models: list[str] | None = None, calib_path: Path | None = None,
    ...
    """High-level entry point (what `tiermoe roofline` and tests call): loads
