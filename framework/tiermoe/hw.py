...
Every entry point that touches a GPU-only code path (tiermoe.collect,
tiermoe.calibrate.compute/transfer/e2e_offload/nf4_fidelity,
tiermoe.sim.backends.cxlmemsim) calls this module first and either (a) runs
...
class Capabilities:
    ...
    def can_collect_traces(self) -> bool:
        """tiermoe.collect (real forward-hook trace collection on live MoE
    ...
    def describe(self) -> str:
        lines = [
            ...
            "-" * 60,
            f"trace collection      -> {'REAL (CUDA)' if self.can_collect_traces else 'UNAVAILABLE here -- use the pilot committed traces under results/, or run tiermoe collect on a CUDA box'}",
            ...
            f"MLP predictor         -> {'CPU/MPS -- runs here, no CUDA needed' if self.can_train_mlp_predictor else 'torch not installed -- pip install tiermoe[predictor]'}",
...
def probe() -> Capabilities:
    ...
    if platform.system() == "Darwin" and platform.machine() == "arm64":
        notes.append(
            ...
            "Only tiermoe.collect and tiermoe.calibrate's GPU submodules require a CUDA box."
        )
    return Capabilities(
