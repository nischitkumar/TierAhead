# Running TierAhead under real CXLMemSim (Linux-only)
...
`tierahead/sim/backends/cxlmemsim.py`'s module docstring for the full
...
`des` backends (`tierahead.sim.backends`), which both reuse ONE precomputed
...
3. Inside the guest, install this framework and run OLMoE's inference path
   ...
   # ... decode loop, per tierahead/collect/collector.py's run_request_single ...
   "
   ```
...
4. Feed the resulting per-epoch latency/bandwidth report back into this
   framework:
   ```python
   from tierahead.sim.backends.cxlmemsim import ingest_cxlmemsim_report
   ...
   `tierahead.report` / the dashboard never need a CXLMemSim-specific code
