...
tierahead.roofline's compute_provenance tagging.
"""
...
from tierahead.calibrate.transfer_fit import fit_from_calib_json
from tierahead.roofline.model import validate_calib_sanity
...
def build_calibration_report(*, compute_path: Path, transfer_path: Path, e2e_path: Path, nf4_path: Path,
    ...
    if compute_data is None:
        provenance_table.append({"parameter": "compute", "status": compute_err,
                                  "detail": "run tierahead.calibrate.compute on a CUDA box"})
    else:
    ...
    if transfer_data is None:
        provenance_table.append({"parameter": "transfer (alpha/beta)", "status": transfer_err,
                                  "detail": "run tierahead.calibrate.transfer_measure on a CUDA box"})
    else:
        try:
    ...
    if e2e_data is None:
        provenance_table.append({"parameter": "e2e offload policy ordering", "status": e2e_err,
                                  "detail": "run tierahead.calibrate.e2e_offload on a CUDA box"})
    else:
    ...
    if nf4_data is None:
        provenance_table.append({"parameter": "nf4 routing fidelity (olmoe)", "status": nf4_err,
                                  "detail": "run tierahead.calibrate.nf4_fidelity on a CUDA box"})
    else:
        nf4_result = nf4_data
