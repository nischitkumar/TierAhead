"""Prompt-set builders. Ported from elp_probe/src/prompts.py. Needs network +
the `datasets`/`huggingface_hub` packages (the `gpu` extra) -- not needed to
import this module, only to call these functions.
"""
...
def build_sharegpt_prompts(n: int, seed: int = 0):
    ...
    try:
        ...
        path = hf_hub_download(repo_id="anon8231489123/ShareGPT_Vicuna_unfiltered",
                                filename="ShareGPT_V3_unfiltered_cleaned_split_no_imsorry.json", repo_type="dataset")
        ds = load_dataset("json", data_files=path, split="train")

    seen, candidates = set(), []
    for row in ds:
        convs = row.get("conversations") or row.get("conversation") or []
        first_human = next((c.get("value", "") for c in convs if c.get("from") in ("human", "user")), None)
        if not first_human:
            continue
...
def build_code_prompts(n: int, seed: int = 0):
    ...
    n_he = min(len(he_prompts), n - n_mbpp)
    ...
    pool = pool[:n]
    return [{"req": f"code-{i:04d}", "domain": "code", "prompt": p, "source": src} for i, (src, p) in enumerate(pool)]
