...
DIFFERENT MODELS:
  ...
  1. "N decode sessions are ALREADY active and self-throttling -- each one
     ...
     exactly what `tierahead.sim.policies.simulate_policy`'s `concurrency`
  ...
  2. "NEW decode sessions arrive over time (not a fixed pool of N), each
     ...
     `tierahead.sim.engine`, an open-loop periodic-arrival model).
...
from tierahead.sim.engine import TrafficClass, pctile_stats, pooled_simulate
