...
what most teams will omit"). ``tierahead.sim.engine`` generalizes that proven
...
contention queueing):
  ...
  tierahead.sim.policies  -- per-layer, trace-driven policy bake-off (none,
  ...
  tierahead.sim.engine    -- the N-class fluid DES for when MULTIPLE traffic
                           ...
                           question, as in tierahead.kv.cotenancy.

``tierahead.sim.backends`` picks which of the above (or the real CXLMemSim
