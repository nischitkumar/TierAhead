...
tierahead.analyze.build_transition_tables (a conditional frequency table,
additive smoothing, ranked by summed log-probability) as a reusable
predictor object with a stable `.topm(...)` API, so tierahead.sim's policies
...
from tierahead.analyze.workload import build_transition_tables
...
class FrequencyTablePredictor:
    ...
    def topm(self, selected_experts_at_l: list[int] | tuple[int, ...], layer: int, d: int, m: int) -> list[int]:
        ...
        if table is None:
            ...
            # should degrade gracefully, matching tierahead.policy.lookahead's
